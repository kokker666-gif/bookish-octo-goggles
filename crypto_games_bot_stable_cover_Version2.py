...
# 1. Ручная настройка авто-подтягивания cover_base_bank по порогу роста в %:
#    - Поля в UI: Enable Auto Pull, Auto pull % (например 0.5 = подтянуть базу если баланс вырос >=0.5% от базы).
#    - Логика: если включено и current_balance >= cover_base_bank * (1 + percent/100) → cover_base_bank = current_balance, логируем.
# 2. Press стратегия (ручные настройки):
#    - Enable Press
#    - Trigger roll ≥ (минимальное значение для активации после WIN)
#    - Trigger payout min/max (диапазон payouts, на которых будем проверять пресс после WIN)
#    - Press bet multiplier (множитель к base_bet на пресс-спине)
#    - Press payout low / high (первый пресс-спин выбирает payout низкий или высокий исходя из roll)
#    - Одна попытка Press занимает ровно 1 следующий спин после активации.
# БОЛЬШЕ НИЧЕГО НЕ МЕНЯЛОСЬ В ОСНОВНОЙ ЛОГИКЕ.
#
# ВНИМАНИЕ: При вводе реальных API ключей делает реальные ставки.
...
# Ставка Cover 50% с учётом house edge и margin
def compute_covering_bet_for_target(
    ...
    """
    Считает ставку для достижения (profit_target * (1+margin_ratio)) при выигрыше на payout.
    Учитывает house edge (уменьшает эффективную выплату).
    """
    try:
...
class CryptoGamesBot:
    def __init__(self, bot_id: str, api: APIClient, config: BetConfig,
        ...
        # Авто-сброс по прибыли
        self.auto_reset_profit_enabled = False
        self.auto_reset_profit_threshold = Decimal("1.0")
        self.auto_reset_profit_triggered = False

        # NEW: авто-подтягивание cover_base_bank по % порогу
        self.cover_auto_pull_enabled = False
        self.cover_auto_pull_percent = Decimal("0")  # если 0 — не подтягиваем (или только вручную)

        # Press стратегия настройки
    ...
    # ---------- Press Strategy Helpers ----------
    def _maybe_activate_press(self):
        if not self.press_enabled:
            return
        if not self._last_spin_win:
            return
        if self.press_active:
            return
        # Проверяем диапазон payout последнего выигрыша
        ...
        # Активируем: выбираем payout (высокий или низкий)
        ...
        self._log(f"[{self.bot_id}] ▶ Press activated next spin payout={self.press_target_payout} bet_mult={self.press_bet_multiplier} (roll={rv})")

    def _consume_press_result(self, win: bool, profit: Decimal):
        # Обработка результата пресс-спина
    ...
    def start(self):
        ...
        while self.is_running:
            ...
            # Press при необходимости активируется основано на предыдущем выигрыше
            self._maybe_activate_press()

            # Определение payout
            if self.press_active and self.press_target_payout is not None:
                payout = self.press_target_payout
            ...
            if win:
                ...
                if self.auto_trigger_min <= payout <= self.auto_trigger_max:
                    ...
                    self._log(f"[{self.bot_id}] ▶ Авто Cover50 старт payout={int(payout)} (2 спина)")

                if self.stop_on_win:
                    self.paused = True

                prefix = "[WIN-PRESS]" if self.press_active else ("[WIN-COVER50]" if (manual_cover or auto_cover) else "[WIN]")
                self._log(f"{prefix} spin {self.spin_count + 1} payout={int(payout)} roll={roll_str} bet={bet:.8f} profit={profit:.8f} edge={self.house_edge_frac:.4f} margin={self.cover_margin_ratio:.4f}")
            else:
                if manual_cover or auto_cover:
...
class BotTab:
    def __init__(self, parent_notebook, manager, bot_index:int, initial_config:BetConfig=None):
        ...
        # NEW: авто-подтягивание cover base
        ...
        # Press frame layout
        press_frame.grid(row=15, column=0, columnspan=2, sticky="w", pady=(10,0))
        self.press_enabled_chk.grid(row=0, column=0, columnspan=2, sticky="w")
        ttk.Label(press_frame, text="Roll ≥").grid(row=1, column=0, sticky="w")
        self.press_roll_min_entry.grid(row=1, column=1, padx=4); self.press_roll_min_entry.insert(0, "98")
        ttk.Label(press_frame, text="Trig payout min").grid(row=2, column=0, sticky="w")
        self.press_payout_min_entry.grid(row=2, column=1, padx=4); self.press_payout_min_entry.insert(0, "5")
        ttk.Label(press_frame, text="Trig payout max").grid(row=3, column=0, sticky="w")
        self.press_payout_max_entry.grid(row=3, column=1, padx=4); self.press_payout_max_entry.insert(0, "50")
        ttk.Label(press_frame, text="Bet x").grid(row=4, column=0, sticky="w")
        self.press_bet_mult_entry.grid(row=4, column=1, padx=4); self.press_bet_mult_entry.insert(0, "5")
        ttk.Label(press_frame, text="Payout low").grid(row=5, column=0, sticky="w")
        self.press_payout_low_entry.grid(row=5, column=1, padx=4); self.press_payout_low_entry.insert(0, "10")
        ttk.Label(press_frame, text="Payout high").grid(row=6, column=0, sticky="w")
        self.press_payout_high_entry.grid(row=6, column=1, padx=4); self.press_payout_high_entry.insert(0, "15")
    ...
    def start_bot(self):
        ...
        try:
            ...
            # Настройки автоподтяжки cover base
            try:
                percent_pull = Decimal(str(self.cover_auto_pull_percent_entry.get()).strip())
                if percent_pull < 0:
            ...
            try:
                self.bot.press_trigger_roll_min = Decimal(str(self.press_roll_min_entry.get()).strip())
            except Exception:
                self.bot.press_trigger_roll_min = Decimal("98")
            try:
                self.bot.press_trigger_payout_min = Decimal(str(self.press_payout_min_entry.get()).strip())
            except Exception:
                self.bot.press_trigger_payout_min = Decimal("5")
            try:
                self.bot.press_trigger_payout_max = Decimal(str(self.press_payout_max_entry.get()).strip())
            except Exception:
                self.bot.press_trigger_payout_max = Decimal("50")
            try:
                self.bot.press_bet_multiplier = Decimal(str(self.press_bet_mult_entry.get()).strip())
                if self.bot.press_bet_multiplier < 1:
                    self.bot.press_bet_multiplier = Decimal("1")
            except Exception:
                self.bot.press_bet_multiplier = Decimal("5")
            try:
                self.bot.press_payout_low = Decimal(str(self.press_payout_low_entry.get()).strip())
            except Exception:
                self.bot.press_payout_low = Decimal("10")
            try:
                self.bot.press_payout_high = Decimal(str(self.press_payout_high_entry.get()).strip())
            except Exception:
            ...
            self.manager.enqueue(('log', bot_id,
                                  f"Запущен payout {int(min_payout)}..{int(max_payout)} press_enabled={self.bot.press_enabled} auto_pull_cover={self.bot.cover_auto_pull_enabled} pull%={self.bot.cover_auto_pull_percent}"))
        except Exception as e:
    ...
    def cover50_action(self):
        if not self.bot or not self.bot.is_running:
            ...
            return
        try:
            cap_pct = Decimal(str(self.cover50_cap_entry.get()).strip())
        except Exception:
            cap_pct = Decimal("2.0")
    ...
    def _process_ui_queue(self):
        ...
        while processed < max_per_cycle:
            try:
                ...
                break
            if typ == 'log':
                # Логи обрабатываются внутри BotTab.log
                pass
            elif typ == 'trace':
                print(payload)
            elif typ == 'bank':
