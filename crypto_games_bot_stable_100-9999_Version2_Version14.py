...
# Функционал:
# - Мульти-бот с вкладками, потоко-безопасная очередь обновления UI.
# - Стратегия линейного payout: от min_payout до max_payout, +1 каждый спин, на WIN — сброс.
# - Фиксированная базовая ставка 0.001 (кроме режимов Cover 50%).
# - Cover 50%:
#     * Ручной (1 спин) — кнопка "Cover 50%".
#     * Авто (2 спина) после любого WIN в диапазоне payout [5..100] (можно расширить).
#     * Собственная база cover_base_bank (независима от других ботов).
#     * Авто‑сброс cover_base_bank по достижении глобальной прибыли (profit_global >= порог).
#     * Работает "немного в плюс": учитывает house edge (Edge из /settings) и добавочный margin (+3% к целевой прибыли).
# - Логи:
#     * В файл: все события, включая результат спина с roll.
#     * В UI: цветные строки (WIN зелёный, LOSS красный).
# - Счётчики прибыли и win/loss для Cover 50% (manual / auto).
# - Возможность ручного сброса cover_base_bank.
#
...
def compute_covering_bet_for_target(
    ...
    # Учёт edge: эффективная прибыль на 1 единицу ставки снижается
    try:
...
class CryptoGamesBot:
    def __init__(self, bot_id: str, api: APIClient, config: BetConfig,
        ...
        self.cover50_cap_ratio = Decimal("0.02")  # максимум 2% банка по умолчанию
        ...
        # Независимая база для расчёта просадки Cover 50%
        ...
        # Edge & margin (для “работы чуть в плюс”)
        self.house_edge_frac = Decimal("0")      # доля edge (0.01 = 1%)
        self.cover_margin_ratio = Decimal("0.03")  # +3% к целевой прибыли
    ...
    def _cover50_bet(self, payout: Decimal, current_balance: Decimal) -> Decimal | None:
        ...
        if dd <= 0:
            return None
        target_profit = dd * Decimal("0.50")  # 50% просадки
    ...
    def start(self):
        ...
        while self.is_running:
            ...
            if win:
                ...
                # Авто Cover триггер (диапазон payout)
                ...
                self._log(f"{prefix} spin {self.spin_count + 1} payout={int(payout)} roll={roll_str} bet={bet:.8f} profit={profit:.8f} edge={self.house_edge_frac:.4f} margin={self.cover_margin_ratio:.3f}")
            else:
                if manual_cover or auto_cover:
    ...
    def start_bot(self):
        ...
        try:
            ...
            self.manager.enqueue(('log', bot_id,
                                  f"Запущен payout {int(min_payout)}..{int(max_payout)} auto_reset_profit={self.bot.auto_reset_profit_enabled} threshold={self.bot.auto_reset_profit_threshold}"))
        except Exception as e:
    ...
    def _process_ui_queue(self):
        ...
        while processed < max_per_cycle:
            try:
                ...
                break
            if typ == 'log':
                # Печать + запись, уже сделано в BotTab.log
                pass
            elif typ == 'trace':
                print(payload)
            elif typ == 'bank':
