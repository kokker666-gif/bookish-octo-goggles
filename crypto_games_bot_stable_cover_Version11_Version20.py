...
# - Добавлена функция `_cover50_bet` для расчёта ставки, покрывающей 50% drawdown, с учётом house edge и маржи.
# - Добавлены отдельные галочки (настройки) для включения стратегии триггера и стратегии восстановления.
# - Добавлены настройки для триггера: % восстановления, диапазон срабатывания (минимум/максимум по Roll).
# - Триггер восстановления срабатывает, как только выпадает число в указанном диапазоне (Roll ∈ [min..max]).
...
class BetConfig:
    ...
    trigger_recovery_percent: Decimal = Decimal("50")       # % покрытия loss_total при триггере
    trigger_roll_min: Decimal = Decimal("90.000")           # нижняя граница Roll для срабатывания триггера
    trigger_roll_max: Decimal = Decimal("99.999")           # верхняя граница Roll для срабатывания триггера
    trigger_payout: Decimal = Decimal("1.50")               # payout для ставок в триггер-режиме (пример: 1.5 ~ шанс ≈ 66%)
...
def compute_covering_bet_for_target(
    ...
    """
    Вычислить ставку для покрытия целевой прибыли с учётом house edge (edge казино) и дополнительной маржи.
    margin_ratio — дополнительная наценка (например 0.03 для +3%).
    """
    try:
        p = Decimal(payout)
    except Exception:
    ...
    if denom <= 0:
        denom = Decimal("1")
    desired = (Decimal(target_profit) / denom) * (Decimal("1") + Decimal(margin_ratio or 0))
    bet = quantize_bet(desired)
    if max_bet_limit is not None:
        try:
            mb = Decimal(str(max_bet_limit))
...
class CryptoGamesBot:
    def __init__(self, bot_id: str, api: APIClient, config: BetConfig,
        ...
        self.periodic_payout_min = Decimal("1.02")   # дефолтный минимум для дробного восстановления
        self.periodic_payout_max = Decimal("2.0")    # дефолтный максимум для дробного восстановления
        ...
        # Триггер восстановления
        self.trigger_enabled = bool(self.config.trigger_enabled)
        self.trigger_recovery_percent_frac = max(Decimal("0"), self.config.trigger_recovery_percent) / Decimal("100")
        self.trigger_roll_min = Decimal(self.config.trigger_roll_min)
        self.trigger_roll_max = Decimal(self.config.trigger_roll_max)
        self.trigger_payout = Decimal(self.config.trigger_payout)
        self.trigger_active_once = False  # включается на ближайшую ставку после попадания в диапазон

        # Глобальный стоп-лосс
        self.global_stop_loss_usdt = Decimal("0.00")
        self._global_sl_active = False

        # Параметры для cover50
        self.cover_margin_ratio = Decimal("0.03")     # +3% маржи к целевой прибыли
        self.cover50_cap_ratio = Decimal("0.5")       # ограничение ставки 50% от баланса
    ...
    def _random_recovery_payout(self) -> Decimal:
        """
        Дробный payout в восстановлении: лестница 1.02, 1.1..2.0. Если диапазон не пересекается — целые значения.
        """
        try:
    ...
    def _calc_cover_drawdown(self, current_balance: Decimal) -> Decimal:
        """
        Абсолютный drawdown: падение от последнего успешного банка или начального.
        """
        base = self.last_successful_bank if self.last_successful_bank not in (None, Decimal("0")) else (self.initial_bank or current_balance)
        try:
            dd = Decimal(base) - Decimal(current_balance)
            if dd < 0:
                dd = Decimal("0")
            return quantize_bet(dd)
        except Exception:
            return Decimal("0")

    def _cover50_bet(self, payout: Decimal, current_balance: Decimal) -> Decimal | None:
        """
        Рассчитать ставку для покрытия 50% drawdown с учётом house edge и дополнительной маржи.
        Ограничить ставку 50% текущего баланса.
        """
        ...
        bet = compute_covering_bet_for_target(
            ...
            min_bet=(self.min_bet or Decimal("0.001")),
            max_bet_limit=self.config.max_bet_limit,
        ...
        try:
            ...
            pass
        if bet < (self.min_bet or Decimal("0.001")):
            bet = (self.min_bet or Decimal("0.001"))
        return bet

    def _check_trigger_activation(self, roll_val) -> bool:
        """
        Проверить попадание Roll в диапазон триггера. Возвращает True, если триггер следует активировать.
        """
        ...
        try:
            if roll_val is None:
                return False
            r = Decimal(str(roll_val))
            return (r >= self.trigger_roll_min and r <= self.trigger_roll_max)
        except Exception:
            return False
    ...
    def start(self):
        ...
        self._log(f"Recovery: payout={self.periodic_payout_min}..{self.periodic_payout_max} enabled={self.config.recovery_enabled} max_spins={self.periodic_recovery_max_spins} SL={self._fmt_money(self.recovery_stop_loss_usdt)} cover%={self._fmt_roll(self.recovery_cover_percent_frac*100)}% random_payout={self.recovery_random_payout}")
        self._log(f"Trigger: enabled={self.trigger_enabled} roll_range=[{self.trigger_roll_min}..{self.trigger_roll_max}] payout={self.trigger_payout} cover%={self._fmt_roll(self.trigger_recovery_percent_frac*100)}%")
        ...
        while self.is_running:
            ...
            # Выбор payout: если активен периодический recovery — используем его payout,
            # иначе — линейная стратегия, ПОСЛЕ чего возможна замена на trigger payout, если триггер активирован.
            if self.periodic_recovery_active:
            ...
            if self.recover66_active and not self.periodic_recovery_active:
            ...
            elif self.periodic_recovery_active and self.config.recovery_enabled:
                ...
                # Основная стратегия или триггер
                # Сначала вычислим базовую цель (основная — покрыть весь loss_total)
                base_target_profit = self.loss_total if not self.base_post_sl_lock else (self.loss_total * self.base_cover_percent_after_sl)

                # Если триггер активен — заменим payout и цель согласно настройкам триггера
                if self.trigger_active_once and self.trigger_enabled:
                    payout = self.trigger_payout
                    base_target_profit = self.loss_total * self.trigger_recovery_percent_frac
                    in_trigger = True
                    self.trigger_active_once = False  # одноразовый триггер на ближайшую ставку
                    self._log(f"[TRIGGER] Используем payout={payout}, cover%={self._fmt_roll(self.trigger_recovery_percent_frac*100)}% для текущей ставки")

                bet = compute_bet_for_target_profit(
                    payout, base_target_profit,
                    self.min_bet or Decimal("0.001"),
                    self.config.max_bet_limit, current_balance,
                    house_edge_frac=self.house_edge_frac
                )
            ...
            # Проверка активации триггера по Roll: включаем на следующую ставку
            if self._check_trigger_activation(roll_val):
                self.trigger_active_once = True
                self._log(f"[TRIGGER] Roll={roll_str} попал в [{self.trigger_roll_min}..{self.trigger_roll_max}] → активируем триггер на следующую ставку")
            ...
            if win:
                ...
                prefix = "[WIN-PR]" if in_periodic else ("[WIN-66%]" if in_recover66 else ("[WIN-TR]" if in_trigger else "[WIN]"))
                ...
                prefix = "[LOSS-PR]" if self.periodic_recovery_active else ("[LOSS-66%]" if self.recover66_active else ("[LOSS-TR]" if in_trigger else "[LOSS]"))
...
class BotTab:
    def __init__(self, parent_notebook, manager, bot_index: int, initial_config: BetConfig = None):
        ...
        # Recovery настройки
        self.rec_enabled_var = tk.BooleanVar(value=True)
        ...
        self.trigger_roll_min_entry = ttk.Entry(left, width=12)
        self.trigger_roll_max_entry = ttk.Entry(left, width=12)
        self.trigger_cover_percent_entry = ttk.Entry(left, width=8)
        self.trigger_payout_entry = ttk.Entry(left, width=12)
        ...
        # Recovery UI
        ttk.Checkbutton(left, text="Включить восстановление", variable=self.rec_enabled_var).grid(row=r, column=0, columnspan=2, sticky="w"); r += 1
        ...
        # Trigger UI
        ttk.Checkbutton(left, text="Включить триггер восстановления", variable=self.trigger_enabled_var).grid(row=r, column=0, columnspan=2, sticky="w"); r += 1
        ttk.Label(left, text="Trigger roll min:").grid(row=r, column=0, sticky="w"); self.trigger_roll_min_entry.grid(row=r, column=1, padx=4); r += 1
        ttk.Label(left, text="Trigger roll max:").grid(row=r, column=0, sticky="w"); self.trigger_roll_max_entry.grid(row=r, column=1, padx=4); r += 1
        ttk.Label(left, text="Trigger cover %:").grid(row=r, column=0, sticky="w"); self.trigger_cover_percent_entry.grid(row=r, column=1, padx=4); r += 1
        ttk.Label(left, text="Trigger payout:").grid(row=r, column=0, sticky="w"); self.trigger_payout_entry.grid(row=r, column=1, padx=4); r += 1
        ...
        # Defaults trigger
        self.trigger_roll_min_entry.insert(0, "90.000")
        self.trigger_roll_max_entry.insert(0, "99.999")
        self.trigger_cover_percent_entry.insert(0, "50")
        self.trigger_payout_entry.insert(0, "1.50")
    ...
    def apply_settings_to_config(self):
        ...
        # Recovery enabled
        cfg.recovery_enabled = bool(self.rec_enabled_var.get())

        # Trigger settings
        cfg.trigger_enabled = bool(self.trigger_enabled_var.get())
        cfg.trigger_roll_min = self._parse_decimal(self.trigger_roll_min_entry.get(), cfg.trigger_roll_min)
        cfg.trigger_roll_max = self._parse_decimal(self.trigger_roll_max_entry.get(), cfg.trigger_roll_max)
        cfg.trigger_recovery_percent = self._parse_decimal(self.trigger_cover_percent_entry.get(), cfg.trigger_recovery_percent)
        cfg.trigger_payout = self._parse_decimal(self.trigger_payout_entry.get(), cfg.trigger_payout)
    ...
    def start_bot(self):
        ...
        try:
            ...
            # Применить настройки восстановления и триггера
            ...
            # Trigger
            self.bot.trigger_enabled = bool(cfg.trigger_enabled)
            self.bot.trigger_roll_min = Decimal(cfg.trigger_roll_min)
            self.bot.trigger_roll_max = Decimal(cfg.trigger_roll_max)
            self.bot.trigger_recovery_percent_frac = max(Decimal("0"), Decimal(cfg.trigger_recovery_percent)) / Decimal("100")
            self.bot.trigger_payout = Decimal(cfg.trigger_payout)
            self.bot.trigger_active_once = False
            ...
            self.manager.enqueue(('log', bot_id,
                                  f"Запущен Base payout {int(min_payout)}..{int(max_payout)}; Recovery(enabled={cfg.recovery_enabled}): range={rec_min}..{rec_max}, max_spins={rec_max_spins}, SL={rec_sl}, cover={rec_cover_pct}%, random_payout=on; Trigger(enabled={cfg.trigger_enabled}): roll_range=[{cfg.trigger_roll_min}..{cfg.trigger_roll_max}], payout={cfg.trigger_payout}, cover={cfg.trigger_recovery_percent}%; Base cover after SL={base_cover_pct}%, Base→Recovery after {base_to_rec_spins} spins; Global SL={global_sl} USDT"))
        except Exception as e:
