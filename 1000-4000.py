...
class SmoothDrawdownPayoutStrategy:
    ...
    L = last_successful_bank - current_balance  (>=0)

    1) Пока L < 50:
       payout растёт плавно 1000 -> 9999 (линейно по L)
       target_pct = 0.25  (N=4: win целится закрыть 25% текущей просадки)

    2) Когда L >= 50:
       payout = 9999
       target_pct = 0.5 (win целится закрыть 50% просадки)

    3) После WIN на payout=9999:
       "сброс на начало": следующий спин принудительно payout=1000 и target_pct=0,
       дальше снова работаем по L.
    """
    def __init__(self,
                 payout_min=Decimal("1000"),
                 payout_max=Decimal("9999"),
                 L_max=Decimal("50"),
                 target_pct_ramp=Decimal("0.25"),
                 target_pct_max=Decimal("0.5")):
        self.payout_min = Decimal(payout_min)
        self.payout_max = Decimal(payout_max)
        self.L_max = Decimal(L_max)
        self.target_pct_ramp = Decimal(target_pct_ramp)
        self.target_pct_max = Decimal(target_pct_max)

        self.target_pct = Decimal("0")
        self._bot = None
        self._force_reset_next_spin = False
        self._last_payout_used = None
    ...
    def _get_drawdown_L(self) -> Decimal:
        if self._bot is None:
            return Decimal("0")

        baseline = self._bot.last_successful_bank
        if baseline is None:
            baseline = self._bot.initial_bank

        # Важно: мы НЕ меняем цикл бота, поэтому берём текущий баланс так:
        current_balance = self._bot.get_current_balance()

        if baseline is None:
            baseline = current_balance

        L = baseline - current_balance
        if L < 0:
            L = Decimal("0")
        return L

    def _payout_from_L(self, L: Decimal) -> Decimal:
        if L <= 0:
            return self.payout_min
        if L >= self.L_max:
            return self.payout_max

        raw = self.payout_min + (self.payout_max - self.payout_min) * (L / self.L_max)
        # payout нужен как целое число
        try:
    ...
    def on_spin_result(self, win: bool):
        # фиксируем последний использованный payout (бот хранит его в _prev_payout)
        if self._bot is not None:
            try:
                self._last_payout_used = self._bot._prev_payout
            except Exception:
                pass

        if win and self._last_payout_used is not None:
            try:
                if Decimal(self._last_payout_used) >= self.payout_max:
                    self._force_reset_next_spin = True
            except Exception:
                pass

    def on_wrap(self):
        # wrap в этой стратегии не используется
        pass

    def next_payout_and_bet(self, state):
        wrapped = False

        # сброс после win на 9999: один спин payout=1000 и target_pct=0
        if self._force_reset_next_spin:
            self._force_reset_next_spin = False
            self.target_pct = Decimal("0")
            payout = self.payout_min
            bet = Decimal(state.get("min_bet", Decimal("0.001"))) or Decimal("0.001")
            return payout, bet, 1, wrapped

        L = self._get_drawdown_L()
        payout = self._payout_from_L(L)

        if payout >= self.payout_max:
            payout = self.payout_max
            self.target_pct = self.target_pct_max  # 50%
        else:
            self.target_pct = self.target_pct_ramp  # 25%
...
class CryptoGamesBot:
    ...
    def set_strategy(self, strategy_obj):
        ...
        # добавлено: чтобы стратегия могла видеть last_successful_bank и баланс
        try:
            if hasattr(self.strategy, "attach_bot"):
                self.strategy.attach_bot(self)
        except Exception:
            pass
        try:
            self.strategy.reset()
    ...
    def start(self):
        ...
        while self.is_running:
            ...
            if self.recovery_active:
                ...
                # добавлено: чтобы стратегия могла видеть, на каком payout был спин
                self._prev_payout = Decimal(payout)
...
class BotTab:
    ...
    def start_bot(self):
        ...
        # заменено: новая стратегия (поля min_payout/max_payout UI не трогаем, но не используем)
        def smooth_factory():
            return SmoothDrawdownPayoutStrategy(
                payout_min=Decimal("1000"),
                payout_max=Decimal("9999"),
                L_max=Decimal("50"),
                target_pct_ramp=Decimal("0.25"),
        ...
        self.bot.set_strategy_factories([smooth_factory])
        self.bot.set_strategy(smooth_factory())
